...
namespace FluentAssertions.Equivalency
{
    ...
    internal class AssertionRule<TSubject, TExpectation> : IAssertionRule
    {
        private readonly Func<IMemberInfo, bool> predicate;
        private readonly Action<IAssertionContext<TSubject, TExpectation>> action;
        private readonly string description;

        public AssertionRule(Expression<Func<IMemberInfo, bool>> predicate, Action<IAssertionContext<TSubject, TExpectation>> action)
        {
            this.predicate = predicate.Compile();
            this.action = action;
            description = "Invoke Action<" + typeof(TSubject).Name + ", " + typeof(TExpectation).Name + "> when " + predicate.Body;
        }
        ...
        public bool AssertEquality(IEquivalencyValidationContext context)
        {
            if (predicate(context))
            {
                ...
                if (subjectIsValidType && expectationIsValidType)
                {
                    action(AssertionContext<TSubject, TExpectation>.CreateFromEquivalencyValidationContext(context));
                }
