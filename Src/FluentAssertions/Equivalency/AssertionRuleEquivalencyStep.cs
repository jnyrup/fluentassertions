...
namespace FluentAssertions.Equivalency
{
    ...
    public class AssertionRuleEquivalencyStep<TSubject, TExpectation> : IEquivalencyStep
    {
        private readonly Expression<Func<IMemberInfo, bool>> canHandle;

        private readonly Action<IAssertionContext<TSubject, TExpectation>> handle;

        public AssertionRuleEquivalencyStep(Expression<Func<IMemberInfo, bool>> predicate, Action<IAssertionContext<TSubject, TExpectation>> action)
        {
            canHandle = predicate;
            handle = action;
        }
        ...
        public bool Handle(IEquivalencyValidationContext context, IEquivalencyValidator parent, IEquivalencyAssertionOptions config)
        {
            ...
            bool expectationIsValidType =
                AssertionScope.Current
                    ...
                    .FailWith("Expected " + context.SelectedMemberDescription + " from expectation to be a {0}{reason}, but found a {1}.",
                        typeof(TExpectation), context.SelectedMemberInfo.MemberType);

            if (subjectIsValidType && expectationIsValidType)
            {
                handle(AssertionContext<TSubject, TExpectation>.CreateFromEquivalencyValidationContext(context));
                return true;
            }
        ...
        public override string ToString()
        {
            return "Invoke Action<" + typeof(TSubject).Name + ", " + typeof(TExpectation).Name + "> when " + canHandle.Body;
        }
    }
}
